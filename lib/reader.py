...
class Reader:
    ...
    def euler_3d(self, file_location, node_s_nr = -1, node_t_nr = -1):
        '''
        Read a file that contains nodes and theire x, y and z coordinats.
        Create a complete undirected graph.
        File format:
        ID_Node X_Position Y_Position Z_Position
        ...
        # read id, x-coordinate, y-coordinate, z-coordinate
        for line in f:
            line_data = line.split()
            ...
            if line_data[0].isdigit():
                #TODO: Check if node_nr is an integer and the numbering is from 
                #1 to n
                node_nr, node_x, node_y, node_z = line_data
            else:
                continue

            temp_node = Node(node_nr, node_x, node_y, node_z)
            nodes.append(temp_node)
        ...
        for node_1 in nodes:
            ...
            for node_2 in nodes:
                if node_1 != node_2:
                    ...
                    temp_distance = math.sqrt((node_1.x - node_2.x)**2 + \
                                    (node_1.y - node_2.y)**2 + (node_1.z - node_2.z)**2)
                    temp_edge = Edge(node_1, node_2, temp_distance)
                    graph.add_edge(temp_edge)
                else:
                    # break, because the graph is undirected
